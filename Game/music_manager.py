...
class MusicPlayer:
    ...
    def play(self, song, loop=None, reset_volume=True, stop_current=True, fade_speed=0):
        ...
        if fade_speed:
            ...
            self.music_player.fade_in(
                song=song, volume=self.default_song_volume, speed=fade_speed
            )
        else:
            song.play()
    ...
    def stop(self, song, fade_speed=0):
        ...
        if fade_speed:
            self.music_player.fade_out(
                song=song, speed=fade_speed, stop=True, reset_volume=True
            )
        else:
            song.stop()
